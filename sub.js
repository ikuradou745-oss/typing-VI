 ...
 */
class SlimeData {
    constructor(id, name, hp, attack, defense, cooldown, range, ability, image, description, cost) {
        this.id = id;
        this.name = name;
        this.hp = hp;               // HP
        this.attack = attack;       // 攻撃力
        this.defense = defense;     // 防御力
        this.cooldown = cooldown;   // 準備タイム (クールダウン秒数)
        this.range = range;         // 射程（内部計算用に数値化）
        this.ability = ability;     // 特殊能力
        this.image = image;         // 画像ファイルパス
        this.description = description; // 説明文
        this.cost = cost;           // 召喚コスト
    }
}
...
class MoneyDisplayController {
    ...
    update(amount) {
        if(this.element) this.element.textContent = amount + "円";
    }
}
